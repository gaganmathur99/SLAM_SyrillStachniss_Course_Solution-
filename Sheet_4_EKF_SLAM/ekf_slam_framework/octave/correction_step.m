...
for i = 1:m
	...
	if(observedLandmarks(landmarkId)==false)
		...
		mu(2*landmarkId+2)=mu(1)+(z(i).range)*cos(z(i).bearing+normalize_angle(mu(3)));
    mu(2*landmarkId+3)=mu(2)+(z(i).range)*sin(z(i).bearing+normalize_angle(mu(3)));
  ...
  del_x=mu(2*landmarkId+2)-mu(1);
  del_y=mu(2*landmarkId+3)-mu(2);
	q=((del_x)^2+(del_y)^2);
  expectedZ(2*i-1)=sqrt(q);
  expectedZ(2*i)=atan2(del_y,del_x)-normalize_angle(mu(3));
	% TODO: Compute the Jacobian Hi of the measurement function h for this observation
	Fj=zeros(5,size(mu,1));
  Fj(1,1)=Fj(2,2)=Fj(3,3)=Fj(4,2*landmarkId+2)=Fj(5,2*landmarkId+3)=1;
  
  Hi=(1/q)*[-sqrt(q)*del_x -sqrt(q)*del_y 0 sqrt(q)*del_x sqrt(q)*del_y ; 
            del_y -del_x -q -del_y del_x]*Fj; 
